...
void GetMinimumPowerSets(vector<string> colorAndNums, int &maxBlue, int &maxRed,
                         int &maxGreen);
...
int main() {
  ...
  for (string game : games) {
    ...
    for (string set : bags) {
      ...
      GetMinimumPowerSets(colorAndNums, maxBlue, maxRed, maxGreen);
    }

    powerSet = maxBlue * maxRed * maxGreen;
    result += powerSet;
  ...
  cout << "Sum of the power sets: " << result << endl;
}
...
void GetMinimumPowerSets(vector<string> colorAndNums, int &maxBlue, int &maxRed,
  ...
  if (map.find("blue") != map.end() && stoi(map["blue"]) > maxBlue) {
    maxBlue = stoi(map["blue"]);
  }

  if (map.find("red") != map.end() && stoi(map["red"]) > maxRed) {
    maxRed = stoi(map["red"]);
  }

  if (map.find("green") != map.end() && stoi(map["green"]) > maxGreen) {
    maxGreen = stoi(map["green"]);
  }
}
